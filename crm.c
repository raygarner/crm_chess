...
add_all_legal_caps(int colour, struct Board *board, struct Action *legal_moves)
{
	...
	while (attacker) {
		index = add_legal_caps(attacker->m, attacker->n, board, 
		                       legal_moves, index); 
...
add_all_legal_moves(int colour, struct Board *board, struct Action *legal_moves)
{
	...
	while (attacker) {
		index = add_legal_moves(attacker->m, attacker->n, board, 
		                        legal_moves, index);
		attacker = attacker->next;
	}
	return index;
                 ...
                 int index)
{
	...
	!will_be_in_check(m, n, dir, 0, board)) {
		...
		m == PAWNHOMEROW(board->all_pieces[m][n].colour)) {
			index = add_one_legal_move(index,m,n,dir*2,0,
			                           legal_moves,board);
		}
	}
	...
	for (i = -1; i <= 1; i += 2) {
		...
		if (is_on_board(m+dir, n+i)) {
			...
			!will_be_in_check(m, n, dir, i, board)) {
				index = add_one_legal_move(index,m,n,dir,i,
				                           legal_moves,board);
			}
			if (piece.type_val == EMPTY && \
                     ...
                     struct Action *legal_moves, int index)
{
	...
	!will_be_in_check(m, n, movem, moven, board))
		index = add_one_legal_move(index,m,n,movem,moven,legal_moves,
		                           board);
	return index;
}
                     ...
                     struct Action *legal_moves, int index)
{
	...
	board->all_pieces[m+movem][n+moven].type_val == EMPTY) {
		if (!will_be_in_check(m,n,movem,moven,board))
			index = add_one_legal_move(index,m,n,movem,moven,
			                           legal_moves,board);
                 ...
                 int index)
{
	...
	for (i = -2; i <= 2; i += 4) {
		if (valid_castle(m,n,0,i,board))
			index = add_one_legal_move(index,m,n,0,i,legal_moves,
			                           board);
	}
	return index;
}
                          ...
                          struct Board *board)
{
	...
	if (explore.a) {
		...
		while (!is_last_action(&legal_moves[i])) {
			...
			execute_cap(legal_moves[i].m, legal_moves[i].n, 
			            legal_moves[i].movem, legal_moves[i].moven, 
			            new_board);
			action = find_most_epic_quiet_move(ab, depth-1, 
			                                   !colour, new_board);
			delete_board(new_board);
			...
			if (explore.a) {
				...
				ab = update_aB(ab,colour,action.eval,
				               &node_type);
				i++;
			}
		}
...
find_most_epic_move(struct Pair ab, int depth, int colour, struct Board *board)
{
	...
	while (!is_last_action(&legal_moves[i])) {
		new_board = copy_board(board);
		execute_move(legal_moves[i].m, legal_moves[i].n, 
		             legal_moves[i].movem, legal_moves[i].moven, 
		             new_board);
...
is_last_action(struct Action *action)
{
	...
	action->movem == LASTACTION.movem && \
	action->moven == LASTACTION.moven && action->eval == LASTACTION.eval;
}
...
king_safety(int m, int n, struct Piece all_pieces[8][8])
{
	...
	for (i = -1; i < 2; i++) {
		if (n+i <= 2 || n+i >= 5) {
			...
			all_pieces[m][n].colour == \
			all_pieces[m+dir][n+i].colour) {
				total += 20;
			}
		}
	}
...
int
valid_bishop_move(int m, int n, int movem, int moven, 
                  struct Piece all_pieces[8][8])
{
                    ...
                    struct Piece all_pieces[8][8])
{
	return all_pieces[m][n].mc == 0 && moven == 0 && \
		...
		all_pieces[m+DIRECTION(all_pieces[m][n].colour)][n].type_val==\
		EMPTY;
                ...
                struct Piece all_pieces[8][8])
{
	...
	if (all_pieces[targetm][targetn].type_val == EMPTY) {
		...
		if (all_pieces[m][n].colour != \
		all_pieces[targetm][targetn].colour && \
...
is_threatened(int m, int n, struct Board *board, int colour)
{
	...
	while (attacker) {
		...
		&& valid_move(attacker->m, attacker->n, m - attacker->m, 
		n - attacker->n, board)) {
			return TRUE;
		}
		attacker = attacker->next;
...
{
	return is_threatened(board->kingm[colour],board->kingn[colour],board,
	                     colour);
}
...
proper_print_board(struct Piece all_pieces[8][8], int colour)
{
	...
	for (m = start; m != end; m+=dir) {
		...
		for (n = start; n != end; n+=dir) {
			if ((m + n) % 2) {
				printf("\033[0;44m"); /* dark gray */
			} else {
				printf("\033[0;47m"); /* light gray */
			}
			piece = all_pieces[m][n];
			printf("%c", get_piece_icon(piece.type_val, 
			                            piece.colour));
			printf(" ");
...
void
copy_piece_list(struct PieceNode *src_head, struct Board *dest_board, 
                int colour)
{
	...
	while (src_head) {
		*dest_node = *src_head; /* copy m and n */
		...
		dest_board->all_pieces[dest_node->m][dest_node->n].node = \
		    dest_node;
		prev_node = dest_node;
...
remove_piece_node(struct Board *board, struct PieceNode *piece_node)
{
	...
	if (piece_node == board->head_piece[colour]) {/* if head */
		neighbour = piece_node->next;
		neighbour->prev = NULL;
		board->head_piece[colour] = neighbour;
	} else if (piece_node == board->tail_piece[colour]) { /* if tail */
...
execute_move(int m, int n, int movem, int moven, struct Board *board)
{
	...
	case PAWN:
		...
		HOMEROW(!board->all_pieces[m][n].colour)) {
		...
		board->all_pieces[m+movem][n+moven].type_val == EMPTY) {
			...
			board->all_pieces[m+movem][n+moven] = \
			    board->all_pieces[m][n];
...
main(int argc, char *argv[])
{
	...
	while (1) {
		...
		start = clock();
		best_move = find_most_epic_move(ab, DEPTH, !computer_colour, 
		                                board);
		end = clock();
